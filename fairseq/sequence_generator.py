...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        finalized = torch.jit.annotate(
            List[List[Dict[str, Tensor]]], [torch.jit.annotate(List[Dict[str, Tensor]], []) for i in range(bsz)],)
...
class EnsembleModel(nn.Module):
    ...
    def forward_encoder(self, net_input: Dict[str, Tensor]):
        net_input.pop('prev_output_pos_tokens')
        if not self.has_encoder():
            return None
        return [model.encoder.forward_torchscript(net_input) for model in self.models]

    @torch.jit.export
    def forward_decoder(self, tokens, encoder_outs, incremental_states, temperature=1.0,):
        log_probs = []
        ...
        for i, model in enumerate(self.models):
            ...
            decoder_out_tuple = (
                decoder_out[0][:, -1:, :].div_(temperature), None if decoder_len <= 1 else decoder_out[1],)
